...
export class Walker extends Enemy {
    constructor(x: number, y: number) {
        super(x, y, "WALKER", "#2ECC71"); // Green color
        this.vx = -0.6; // slightly faster
    }
    ...
    draw(ctx: CanvasRenderingContext2D, camX: number) {
        ...
        const drawX = this.x - camX;
        const drawY = this.y;
        const bounce = Math.sin(this.walkTimer / 8) * 3;
        ...
        // Body (Walker) - Rounded top
        ctx.fillStyle = this.color;
        ctx.beginPath();
        ctx.moveTo(0, this.h);
        ctx.lineTo(this.w, this.h);
        ctx.lineTo(this.w, 10);
        ctx.quadraticCurveTo(this.w / 2, -5, 0, 10);
        ctx.closePath();
        ctx.fill();
        ...
        // Pupils (looking in direction of movement)
        ctx.fillStyle = "#000000";
        const pupilOffset = this.vx > 0 ? 2 : -2;
        ctx.beginPath();
        ctx.arc(10 + pupilOffset, 14, 2, 0, Math.PI * 2);
        ctx.arc(22 + pupilOffset, 14, 2, 0, Math.PI * 2);
        ctx.fill();

        // Feet
        ctx.fillStyle = "#000000";
        const foot1 = Math.sin(this.walkTimer / 8) > 0 ? 0 : -4;
        const foot2 = Math.sin(this.walkTimer / 8) < 0 ? 0 : -4;
        ctx.fillRect(4, this.h - 4 + foot1, 8, 4);
        ctx.fillRect(20, this.h - 4 + foot2, 8, 4);
