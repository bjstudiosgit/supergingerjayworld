...
export class Bouncer extends Enemy {
    ...
    constructor(x: number, y: number) {
        ...
        this.w = 64;
        this.h = 64;
        this.vx = -0.8; // Slower than before
    }
    ...
    update() {
        ...
        if (this.hp <= 0) {
            ...
            // Stay stunned for 2 seconds, then charge off to the right
            if (this.defeatTimer > 120) {
                this.vx = 12; // Run off fast!
                this.x += this.vx;
            } else {
                this.vx = 0;
            }
        ...
        // Bully-like jump logic: Slower frequency
        this.jumpTimer++;
        if (this.jumpTimer % 180 === 0 && this.onGround) {
            this.vy = -10;
            this.onGround = false;
        }
