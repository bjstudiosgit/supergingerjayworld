...
export const generateMap = (): string[] => {
    ...
    // Large structure with hidden coin area
    block(75, 4, 10, 1, 'B');
    place(78, 3, 'ccc');
    block(75, 10, 10, 1, 'B');
    place(78, 11, 'ccc');
    ...
    const gaps = [95, 96, 97, 120, 121, 122, 145, 146, 147];
    gaps.forEach(x => {
        map[14][x] = '.';
        map[15][x] = '.';
    });
    ...
    // PEACOCKS GYM (Transition to 1-3)
    block(242, 4, 15, 10, 'S'); // Gym Structure
    block(247, 9, 5, 5, 'C');   // Entrance area (marked as 'C' for logic)
    
    // Sign area (We'll use Bricks to make it stand out)
    block(242, 4, 15, 1, 'B'); 
