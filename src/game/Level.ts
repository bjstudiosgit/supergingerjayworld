...
export function setCurrentLevel(level: LevelId) {
    ...
    if (level === '1-1') currentLevelMap = Level1_1.generateMap();
    else if (level === '1-1-cave') currentLevelMap = Level1_1_Cave.generateMap();
    else if (level === '1-2') currentLevelMap = Level1_2.generateMap();
    else if (level === '1-3') currentLevelMap = Level1_3.generateMap();
}
    ...
    currentLevelMap.forEach((row, y) => {
        row.split("").forEach((char, x) => {
            if (char === "#") tiles.push({ x: x * TILE, y: y * TILE, w: TILE, h: TILE, type: 'ground' });
                ...
                const tile: Tile = { x: x * TILE, y: y * TILE, w: TILE, h: TILE, type: 'question' };
                if (currentLevelId === '1-1' && x === 23 && y === 9) {
                    tile.hits = 10;
                }
                tiles.push(tile);
            }
                ...
                const hasAirAbove = y > 0 && currentLevelMap[y-1][x] === '.';
                const hasAirBelow = y < currentLevelMap.length - 1 && currentLevelMap[y+1][x] === '.';
                const isTop = hasAirAbove;
                const isBottom = hasAirBelow;
                const isLeft = x > 0 && currentLevelMap[y][x-1] !== 'P';
                tiles.push({ x: x * TILE, y: y * TILE, w: TILE, h: TILE, type: 'pipe', isTop, isBottom, isLeft });
            }
            else if (char === "L") {
...
export function getClouds(): Cloud[] {
    if (currentLevelId === '1-1-cave') return [];
...
export function getScenery(): Scenery[] {
    ...
    if (currentLevelId === '1-3') {
        // Level 1-3 pattern: Sparse ground scenery, focus on verticality
        for (let i = 0; i < 8000; i += 64 * TILE) {
            scenery.push({ x: i + 10 * TILE, y: 14 * TILE, type: 'hill' });
            scenery.push({ x: i + 40 * TILE, y: 14 * TILE, type: 'bush' });
        }
    } else {
        // Classic 1-1 scenery pattern
        for (let i = 0; i < 7000; i += 48 * TILE) {
            // Small bush
