...
export const MobileControls: React.FC<MobileControlsProps> = ({ onControl }) => {
    ...
    const dpadBtn = `${btnBase} w-[60px] h-[60px] text-white text-2xl font-black`;
    const actionBtn = `${btnBase} w-[75px] h-[75px] text-white text-3xl font-black italic`;
    ...
    return (
        <div className="fixed inset-0 pointer-events-none z-50 select-none md:hidden font-mono">
            ...
            <div className="absolute bottom-10 right-8 pointer-events-auto flex items-end gap-4 pb-4">
                <div className="flex flex-col items-center gap-8">
                   <div className="relative">
                        ...
                        <span className="absolute -top-5 left-1/2 -translate-x-1/2 text-white/50 text-[9px] font-bold uppercase tracking-widest">Run</span>
                   </div>
                </div>
                <div className="flex flex-col items-center gap-8 translate-y-[-30px]">
                    <div className="relative">
                        <button 
                            className={`${actionBtn} bg-yellow-500/40 border-yellow-400/50 scale-110 shadow-[0_0_25px_rgba(255,255,255,0.2)]`}
                            onTouchStart={(e) => handleTouchStart('Space', e)} 
                            onTouchEnd={(e) => handleTouchEnd('Space', e)}
                        >
                            A
                        </button>
                        <span className="absolute -top-5 left-1/2 -translate-x-1/2 text-white/50 text-[9px] font-bold uppercase tracking-widest">Jump</span>
                    </div>
                </div>
            </div>
        </div>
