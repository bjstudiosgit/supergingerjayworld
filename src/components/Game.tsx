...
const getEnemiesForLevel = (levelId: LevelId): EnemyEntity[] => {
    if (levelId === '1-1') {
        return [
            new Enemy(22 * 32, 16 * 32, "MARGS", "#8E44AD"),
            new Enemy(40 * 32, 16 * 32, "MARGS", "#8E44AD"),
            new Enemy(50 * 32, 16 * 32, "MARGS", "#8E44AD"),
            new Enemy(52 * 32, 16 * 32, "MARGS", "#8E44AD"),
            new Enemy(93 * 32, 8 * 32, "MARGS", "#8E44AD"),
...
export default function Game() {
    ...
    // Internal resolution: Taller for mobile to feel like a Gameboy
    const canvasWidth = isMobile ? 800 : 1024;
    const canvasHeight = isMobile ? 1000 : 640;
    ...
    useEffect(() => {
        ...
        const loop = (currentTime: number) => {
            ...
            if (elapsed > fpsInterval) {
                ...
                collideTiles(player, tiles, (t) => {
                    ...
                    if (t.type === 'question') {
                        ...
                        // 🎲 Dynamic PowerUp Spawning: If player is small, 40% chance of a power-up
                        else if (player.state === "small" && Math.random() < 0.4) {
                            shouldSpawnPowerUp = true;
                        }
                    }
                ...
                if (keys['ArrowDown'] && player.onGround && currentLevelId === '1-1') {
                    ...
                    if (player.x + player.w / 2 > pipeX && player.x + player.w / 2 < pipeX + 2 * TILE) {
                        if (Math.abs(player.y + player.h - pipeY) < 5) {
                            ...
                            gameRef.current.player.x = 3 * TILE;
                            gameRef.current.player.y = 2 * TILE;
                            gameRef.current.player.vy = 0;
                ...
                if (keys['ArrowDown'] && player.onGround && currentLevelId === '1-1-cave') {
                    ...
                    if (player.x + player.w / 2 > pipeX && player.x + player.w / 2 < pipeX + 2 * TILE) {
                        if (Math.abs(player.y + player.h - pipeY) < 5) {
                            ...
                            if (character === "ginja") playSound('gingerjesus');
                            else if (character === "ryno") playSound('ryno_itstime');
                            else if (character === "tymeless") playSound('tymeless_letsgoo');
            ...
            // Camera
            if (gameRef.current.levelTransitionState === 'none') {
                updateCamera(player, canvas.width, keys);
            }
            ...
            for (let t of tiles) {
                ...
                if (t.type === 'ground') {
                    ...
                    // Texture/Details
                    ctx.fillStyle = isCave ? "#50eaff" : "#d85c1c";
                    ctx.fillRect(t.x - camera.x + 4, t.y + 10, 4, 4);
                    ctx.fillRect(t.x - camera.x + 20, t.y + 25, 6, 6);
                    ctx.fillRect(t.x - camera.x + 30, t.y + 12, 4, 4);
                    ...
                    // Brick highlights
                    ctx.fillStyle = isCave ? "#50eaff" : "#d85c1c";
                    ctx.fillRect(t.x - camera.x + 2, t.y + 2, t.w - 4, 2);
    ...
    return (
        <div className="relative w-screen h-screen overflow-hidden bg-[#000]">
                ...
                <div className="absolute inset-0 z-10 flex items-center justify-center bg-black">
                    ...
                    >
                        ...
                        <div className="md:hidden absolute inset-0 z-10 flex flex-col items-center justify-center bg-[#333] overflow-hidden">
                            ...
                            {/* 3x3 Grid for Mobile */}
                            <div className="grid grid-cols-3 grid-rows-3 gap-2 w-full max-w-sm h-[60%]">
                                {[
                                    ...
                                    <button 
                                        ...
                                        className={`border flex flex-col items-center justify-center p-1 transition-all ${character === char.id ? "border-yellow-400 bg-yellow-400/20 scale-105" : "border-white/20 bg-black/20 hover:border-white/50"}`}
                                    >
                                        <img 
                                            ...
                                            style={{ imageRendering: 'pixelated' }} 
                                        />
                                        <span className="text-white font-bold text-[10px] uppercase truncate w-full text-center">{char.name}</span>
                                    </button>
                                ))}
                            </div>
            ...
            <div className={`flex-1 flex ${isMobile ? 'items-start pt-10 p-4' : 'items-center p-0'} justify-center w-full h-full relative pointer-events-none`}>
                <canvas 
                    ref={canvasRef} 
                    width={canvasWidth} 
                    height={canvasHeight} 
                    ...
                    className={`block pointer-events-auto ${isMobile ? 'rounded-lg border-4 border-white/10' : 'rounded-none border-none'} ${(currentLevelId.includes('cave') || currentLevelId === '1-2') && camera.x < 225 * TILE ? 'bg-[#000]' : 'bg-[#5C94FC]'}`} 
                />
            </div>
            {screen === 'game' && (
