...
import { Enemy, Bouncer, Cookie } from '../game/Enemy';
...
const getEnemiesForLevel = (levelId: LevelId): EnemyEntity[] => {
    if (levelId === '1-1') {
        ...
        return [
            // Intro
            new Enemy(10 * TILE, 13 * TILE, "MARGS", "#8E44AD"),
            ...
            // Ground section
            new Enemy(85 * TILE, 13 * TILE, "MARGS", "#8E44AD"),
            new Enemy(95 * TILE, 13 * TILE, "MARGS", "#8E44AD"),
            ...
            // Transition
            new Enemy(160 * TILE, 13 * TILE, "MARGS", "#8E44AD"),
    ...
    useEffect(() => {
        ...
        const loop = () => {
            ...
            if (gameRef.current.levelTransitionState === 'none') {
                ...
                // Pipe transition (1-1 to 1-2) via the later pipe
                if (keys['ArrowDown'] && player.onGround && currentLevelId === '1-1') {
                    const pipeX = 163 * TILE;
                    const pipeY = 12 * TILE;
                    // Check if player's center is within the pipe's width and player is standing on top of it
                    if (player.x + player.w / 2 > pipeX && player.x + player.w / 2 < pipeX + 2 * TILE) {
                        if (Math.abs(player.y + player.h - pipeY) < 5) {
                            setCurrentLevel('1-2');
                            gameRef.current.tiles = getTiles();
                            gameRef.current.enemies = getEnemiesForLevel('1-2');
                            gameRef.current.player.x = 2.5 * TILE; // Center under the ceiling pipe
                            gameRef.current.player.y = 6 * TILE; // Drop out of the pipe
                            gameRef.current.player.vy = 2;
                            applyLevelSettings('1-2');
                        }
                    }
                }
                ...
                // Exit pipe transition (1-2 to 1-3)
                if (player.onGround && currentLevelId === '1-2') {
                    const exitPipeX = 189 * TILE;
                    if (player.x > exitPipeX) {
                        setCurrentLevel('1-3');
                        gameRef.current.tiles = getTiles();
                        gameRef.current.enemies = getEnemiesForLevel('1-3');
                        gameRef.current.player.x = 100;
                        gameRef.current.player.y = 480; 
                        applyLevelSettings('1-3');
                    }
                }
            ...
            if (gameRef.current.levelTransitionState !== 'none') {
                if (gameRef.current.levelTransitionState === 'sliding') {
                    ...
                    if (currentLevelId === '1-2') castleEntranceX = 249.5 * TILE;
            ...
            for (let t of tiles) {
                ...
                if (t.type === 'ground') {
                    ...
                    const relX = tx - 202; // Left side of castle
            ...
            const isEnteringCastle = gameRef.current.levelTransitionState === 'walking' && player.x >= 203.8 * TILE;
            if (!isEnteringCastle) {
